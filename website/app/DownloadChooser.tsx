...
export default function DownloadChooser() {
  ...
  const recommendedArchitecture =
    source === 'conflict' ? 'unknown' : architecture;
  ...
  return (
    <div
      className="download-chooser"
      id="downloads"
      aria-labelledby="download-chooser-title"
    >
      <fieldset className="download-options">
        <legend className="visually-hidden" id="download-chooser-title">
          选择 Tunnelful 下载版本
        </legend>
        <a
        ...
        >
          ...
          <small>
            arm64
            <span
              className="recommend-mark"
              data-on={recommendedArchitecture === 'arm64' ? '' : undefined}
              aria-hidden={recommendedArchitecture !== 'arm64'}
            >
              · 推荐
            </span>
          </small>
        ...
        >
          ...
          <small>
            ...
            <span
              className="recommend-mark"
              data-on={recommendedArchitecture === 'x86_64' ? '' : undefined}
              aria-hidden={recommendedArchitecture !== 'x86_64'}
            >
              · 推荐
            </span>
          </small>
      ...
      <p className="download-meta">
        ...
        <span aria-hidden="true">·</span>
        <span>macOS 14 及更高版本</span>
      </p>
      <p className="installer-note">
        尚未经过 Apple 公证；首次打开时，macOS
        可能需要你在“隐私与安全性”中确认。
      </p>
      <details className="migration-note">
        ...
        <p>
          旧版更新窗口只会引导前往 Release
          页面。若启用了登录项，请先在旧版设置中关闭；退出并手动安装{' '}
          {releaseVersion}
          后，再按需重新开启。其余偏好设置会自动迁移。0.1.10 至 0.1.13
          可直接使用应用内更新。
        </p>
      </details>
    </div>
