# Tunnelful 0.1.13 设计校验

## 验收输入

- 官网参考图（仓库内可复核）：`website/public/tunnelful-window-v0.1.10.png` / `website/public/tunnelful-window-dark-v0.1.10.png`
...
- 0.1.13 沿用的官网深色外观基准：`website/public/tunnelful-window-dark-v0.1.10.png`
- 0.1.13 沿用的官网浅色外观基准：`website/public/tunnelful-window-v0.1.10.png`
...
0.1.13 有小范围界面行为变化：保存 Ingress 后弹出 DNS 确认，必要时再询问重启或启动 Tunnel。窗口外壳、侧栏分组与卡片布局未改，因此继续沿用 0.1.10 已验收截图，不重新生成官网基准图。
...
- 0.1.10 官网截图来自 Xcode 26 / macOS 26 SDK 构建的当前窗口外壳，但 0.1.10 正式 DMG 使用旧 SDK 构建，导致同一原生 `NavigationSplitView` 在 macOS 26 上仍呈现旧式侧栏。
- 0.1.11 起正式包使用 Xcode 26 与 macOS 26 SDK；0.1.13 继续该工具链，窗口外观基准仍为现有两张截图。
- 最终验收必须从 0.1.13 的公开 DMG 启动应用并核对构建 SDK、侧栏，以及保存后的 DNS 确认与 Tunnel 重启/启动询问，不得用本地 Debug App、源码截图或网站图片代替安装包验收。
- macOS 14 与 macOS 15 会按各自系统版本呈现原生控件，外观不要求逐像素等同于 macOS 26 截图，但必须保持功能、布局可用且无裁切。

## 结论与边界

- README 与官网继续使用已验收的浅色、深色 macOS 26 外观基准；官网图片与对应 `design/actual-app*.png` 逐字节一致。0.1.13 未改窗口外壳，因此未重拍截图。
