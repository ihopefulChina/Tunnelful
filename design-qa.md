...
- 0.1.15 沿用的官网深色外观基准：`website/public/tunnelful-window-dark-v0.1.10.png`
- 0.1.15 沿用的官网浅色外观基准：`website/public/tunnelful-window-v0.1.10.png`
...
0.1.15 的窗口改回系统侧栏和控件背景，分组与卡片布局未改。官网继续沿用 0.1.10 已验收截图，不重新生成基准图。标志在深色页面使用抬高底板的 `tunnelful-icon-dark.png`。
...
- 0.1.11 起正式包使用 Xcode 26 与 macOS 26 SDK；0.1.15 继续该工具链，窗口外观基准仍为现有两张截图。
- 最终验收必须从 0.1.15 的公开 DMG 启动应用并核对构建 SDK、侧栏，以及单实例、同名 Tunnel 互斥与发布/DNS 确认，不得用本地 Debug App、源码截图或网站图片代替安装包验收。
- macOS 14 与 macOS 15 会按各自系统版本呈现原生控件，外观不要求逐像素等同于 macOS 26 截图，但必须保持功能、布局可用且无裁切。

## 结论与边界

- README 与官网继续使用已验收的浅色、深色 macOS 26 外观基准；官网图片与对应 `design/actual-app*.png` 逐字节一致。0.1.15 未改窗口外壳，因此未重拍截图。
