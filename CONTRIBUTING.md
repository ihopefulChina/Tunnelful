...
感谢你参与 Tunnelful。项目当前源码版本为 `0.1.13`，优先接受能够提升可靠性、安全性、可访问性和基础 Tunnel 工作流的聚焦改动。

当前 `0.1.13` 构建流程使用 Xcode 26 与 macOS 26 SDK，分别生成 Apple 芯片 `arm64` 与 Intel `x86_64` 单架构应用，最低部署目标仍为 macOS 14。
正式应用的 Bundle ID 为 `app.ihopeful.Tunnelful`。修改 Bundle ID 会影响偏好设置、登录项与 Sparkle 更新兼容性，不能作为普通重命名提交。
...
- Apple 芯片或 Intel Mac
- macOS 14 或更高版本
- 支持 Swift 5.10 的 Xcode；正式发布和 macOS 26 外观验收必须使用 Xcode 26 与 macOS 26 SDK
