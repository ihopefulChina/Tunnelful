...
本文供 Tunnelful 维护者使用。所有发布均应从干净的 `main` 分支生成。`0.1.13` 分别提供 Apple 芯片 `arm64` 与 Intel `x86_64` 单架构安装包，两者均采用 ad-hoc 签名且未公证。更新源由 Sparkle appcast 提供。
...
- `https://github.com/ihopefulChina/Tunnelful/releases/latest` 跳转到 `v0.1.13`，且 `https://github.com/ihopefulChina/Tunnelful/releases/tag/v0.1.13` 可访问。
- `Tunnelful-0.1.13-arm64.dmg`、`Tunnelful-0.1.13-x86_64.dmg`、两份 `.sha256` 与 `appcast.xml` 五个 Release 附件均可下载。
...
- `https://ihopefulchina.github.io/Tunnelful/` 展示 0.1.13 文案和上述分组侧栏基准，浅色与深色主题均使用对应图片，两个架构按钮指向 0.1.13 附件。
...
- 更新源的两个架构条目均为 0.1.13，Intel 内部版本为 32、Apple 芯片内部版本为 33；下载 URL、文件长度、EdDSA 签名和公开附件一致，两个条目都包含 `informationalUpdate` 与 `belowVersion=26`。Apple 芯片条目必须声明 `sparkle:hardwareRequirements=arm64`，Intel 条目必须声明 `sparkle:hardwareRequirements=x86_64`。
- 在 0.1.9 Intel build 24 与 Apple 芯片 build 25 上分别检查更新：只能看到迁移说明和 Release 网页入口，不得下载或尝试安装 0.1.13；在 0.1.10 build 26/27、0.1.11 build 28/29 与 0.1.12 build 30/31 上应能正常发现并安装 0.1.13，在 0.1.13 build 32/33 上不得把当前版本误报为新版本。
