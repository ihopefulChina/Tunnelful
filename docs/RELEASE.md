...
本文供 Tunnelful 维护者使用。所有发布均应从干净的 `main` 分支生成。`0.1.10` 分别提供 Apple 芯片 `arm64` 与 Intel `x86_64` 单架构安装包，两者均采用 ad-hoc 签名且未公证。更新源由 Sparkle appcast 提供。
...
两个身份继续共用 `/Tunnelful/appcast.xml`，但 0.1.10 条目必须包含 `sparkle:informationalUpdate`，并以 `sparkle:belowVersion` 设为 `26`。这样旧版 Intel build 24 与 Apple 芯片 build 25 只显示迁移说明和 Release 网页入口，不会下载或安装 Bundle ID 不同的应用；0.1.10 Intel build 26、Apple 芯片 build 27 及后续版本仍使用正常 Sparkle 更新。不得删除这条版本门槛，除非已经设计并验证新的迁移路径。
...
- `https://github.com/ihopefulChina/Tunnelful/releases/latest` 跳转到 `v0.1.10`，且 `https://github.com/ihopefulChina/Tunnelful/releases/tag/v0.1.10` 可访问。
- `Tunnelful-0.1.10-arm64.dmg`、`Tunnelful-0.1.10-x86_64.dmg`、两份 `.sha256` 与 `appcast.xml` 五个 Release 附件均可下载。
- 两个 DMG 的公开下载字节数和 SHA-256 与 Release 附件一致，应用内 `CFBundleIdentifier` 均为 `app.ihopeful.Tunnelful`。
- `https://ihopefulchina.github.io/Tunnelful/` 展示 0.1.10 文案和分组侧栏截图，浅色与深色主题均使用对应图片，两个架构按钮指向 0.1.10 附件。
...
- 更新源的两个架构条目均为 0.1.10，下载 URL、内部版本、文件长度与 EdDSA 签名和公开附件一致；两个条目都包含 `informationalUpdate` 与 `belowVersion=26`。
- 在 0.1.9 Intel build 24 与 Apple 芯片 build 25 上分别检查更新：只能看到迁移说明和 Release 网页入口，不得下载或尝试安装 0.1.10；在 0.1.10 build 26/27 上检查更新不得把当前版本误报为新版本。
